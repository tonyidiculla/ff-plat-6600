-- Fix column names in the CRUD function based on actual table schema

-- Drop the function with incorrect column names
DROP FUNCTION IF EXISTS public.crud_entity_master_l01;

-- Create the correct CRUD function based on actual table columns
CREATE OR REPLACE FUNCTION public.crud_entity_master_l01(
    ...
    -- Actual table columns as parameters (based on the data we see)
...
BEGIN
    ...
    CASE operation
        WHEN 'CREATE' THEN
            ...
            -- Insert new entity (only include columns that exist)
        ...
        WHEN 'UPDATE' THEN
            ...
            -- Update entity with only existing columns
...
-- Add comment
COMMENT ON FUNCTION public.crud_entity_master_l01 IS 'Comprehensive CRUD function for global.entity_master_l01 table with correct column names';