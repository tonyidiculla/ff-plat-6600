-- Fix type mismatches in the CRUD function

-- Drop the function with type issues
DROP FUNCTION IF EXISTS public.crud_entity_master_l01;

-- Create the corrected CRUD function with proper type handling
CREATE OR REPLACE FUNCTION public.crud_entity_master_l01(
    ...
    p_assigned_manager_id TEXT DEFAULT NULL,  -- Changed to TEXT to avoid UUID conversion issues
    ...
    p_created_by TEXT DEFAULT NULL,  -- Changed to TEXT to avoid UUID conversion issues
    p_updated_by TEXT DEFAULT NULL,  -- Changed to TEXT to avoid UUID conversion issues
    p_deleted_by TEXT DEFAULT NULL,  -- Changed to TEXT to avoid UUID conversion issues
...
BEGIN
    ...
    -- Convert text UUIDs to proper UUIDs where needed, handling NULL and invalid values
    BEGIN
    ...
    BEGIN
        deleted_by_uuid := CASE 
            WHEN p_deleted_by IS NULL OR p_deleted_by = '' THEN NULL
            ELSE p_deleted_by::UUID
        END;
    EXCEPTION WHEN invalid_text_representation THEN
    ...
    CASE operation
        WHEN 'CREATE' THEN
            ...
            -- Insert new entity with safe type conversion
            INSERT INTO global.entity_master_l01 (
                ...
                p_contact_person, p_billing_email, assigned_manager_uuid, 
        ...
        WHEN 'UPDATE' THEN
            ...
            -- Update entity with safe type handling
            UPDATE global.entity_master_l01 SET
                ...
                assigned_manager_id = COALESCE(assigned_manager_uuid, assigned_manager_id),
                ...
                updated_by = COALESCE(updated_by_uuid, updated_by),
                updated_at = NOW()
            WHERE entity_id = p_entity_id
            RETURNING * INTO entity_record;
        ...
        WHEN 'DELETE' THEN
            ...
            -- Soft delete with safe type handling
            UPDATE global.entity_master_l01 SET
                deleted_at = COALESCE(p_deleted_at, NOW()),
                deleted_by = COALESCE(deleted_by_uuid, deleted_by),
...
-- Add comment
COMMENT ON FUNCTION public.crud_entity_master_l01 IS 'Comprehensive CRUD function for global.entity_master_l01 table with proper type handling';