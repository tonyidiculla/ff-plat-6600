-- Migration: Remove applicable_solution_types column from coupons table
-- Date: 2025-09-26
-- Purpose: Remove applicable_solution_types column as coupons are intended for entities, not solution types

-- Remove the column from the coupons table
ALTER TABLE global.coupons DROP COLUMN IF EXISTS applicable_solution_types;

-- Also remove from public.coupons if it exists there
ALTER TABLE public.coupons DROP COLUMN IF EXISTS applicable_solution_types;

-- Update the crud_coupons function to remove applicable_solution_types references
...
-- Grant permissions
REVOKE ALL ON FUNCTION public.crud_coupons(text, jsonb, uuid, jsonb) FROM public;
