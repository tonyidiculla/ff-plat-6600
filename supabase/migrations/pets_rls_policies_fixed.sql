-- ============================================================
-- RLS POLICIES FOR public.pets TABLE (CORRECTED VERSION)
-- ============================================================
-- This file contains all Row Level Security policies for the pets table
-- IMPORTANT: RLS policies MUST use explicit schema prefixes (public.profiles)
-- The client's schema configuration does NOT affect RLS policy evaluation
...
-- NOTE: Using explicit 'public.profiles' for proper PostgreSQL resolution
...
2. PET OWNERS (authenticated)
   ...
   - Ownership determined by profiles.user_platform_id = pets.user_platform_id
...
- RLS policies execute in PostgreSQL context, not Supabase client context
- ALWAYS use explicit schema prefixes in RLS policies (e.g., 'public.profiles')
- Client's db.schema setting only affects query routing, not RLS evaluation
- All table references in USING/WITH CHECK clauses must be fully qualified
- This ensures PostgreSQL can resolve table names correctly during RLS evaluation
*/
