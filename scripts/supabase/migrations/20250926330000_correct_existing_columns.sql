-- Create CRUD function using ONLY columns that actually exist in the table
...
CREATE OR REPLACE FUNCTION public.crud_entity_master_l01(
    ...
    -- ONLY parameters for columns that actually exist in the table
    ...
    p_assigned_manager_id UUID DEFAULT NULL,  -- exists but nullable
    ...
    p_is_active TEXT DEFAULT NULL,  -- text, not boolean
    p_organization_id UUID DEFAULT NULL,
    p_organization_platform_id TEXT DEFAULT NULL,
    p_created_by UUID DEFAULT NULL,  -- exists but nullable
    -- NOTE: updated_by DOES NOT EXIST in table - removed
    p_deleted_by UUID DEFAULT NULL,  -- exists but nullable
...
BEGIN
    ...
    CASE operation
        ...
        WHEN 'UPDATE' THEN
            ...
            -- Update using ONLY columns that exist in the table
            UPDATE global.entity_master_l01 SET
                ...
                -- updated_by field REMOVED because it doesn't exist
                updated_at = NOW()
            WHERE entity_id = p_entity_id
        ...
        WHEN 'CREATE' THEN
            ...
            INSERT INTO global.entity_master_l01 (
                ...
                emergency_contact_number, is_regulated, 
                ...
                p_is_regulated, p_logo_storage, p_notes, 
...
-- Comment
COMMENT ON FUNCTION public.crud_entity_master_l01 IS 'CRUD function using only columns that actually exist in the entity_master_l01 table';